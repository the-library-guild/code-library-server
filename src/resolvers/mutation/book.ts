...
const linkQr = async (_: any, { qrId, mediaId }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    const qrDoc = await qrController.create(qrId, mediaId);

    if (!qrDoc) throw new ApolloError("Failed to create qr code", "Error");
...
const createBook = async (_: any, { bookData }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    const bookDoc = await itemController.createBook(bookData);
...
const updateBook = async (_: any, { bookId, bookData }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    const bookDoc = await itemController.patch(bookId, bookData);
...
const deleteBook = async (_: any, { bookId }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    const bookDoc = await itemController.delete(bookId);
...
const rentBook = async (_: any, { bookId }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    const dings = await userController.rentBook(user.email, bookId);

    // const bookDoc = await itemController.get(bookId);

    // let userDoc = await userController.get(user.email);
...
const returnBook = async (_: any, { bookId }: any, { user }: any) =>
  handleErrs(async () => {
    await userController.returnBook(user.email, bookId);
    // const bookDoc = await itemController.get(bookId);
    // const userDoc = await userController.get(user.email);
    ...
    //   !userDoc.childrenIds.includes(bookId)
    // )
    //   throw new ApolloError(
...
const processBook = async (_: any, { bookId }: any, { user }: any) =>
  handleErrs(async () => {
    ...
    await userController.processBook(user.email, bookId);
